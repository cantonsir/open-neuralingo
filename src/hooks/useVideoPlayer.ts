...
export function useVideoPlayer({
  ...
  // Clear loop and segment state when video changes
  useEffect(() => {
  ...
  const fetchSubtitles = async (id: string, shouldNavigate: boolean = true) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handle both old format (array) and new format (object with subtitles + metadata)
        const subtitlesData = Array.isArray(data) ? data : data.subtitles;
        const metadata = Array.isArray(data) ? undefined : data.metadata;

        const parsed: Subtitle[] = subtitlesData.map((item: any) => ({
          id: Math.random().toString(36).substr(2, 9),
          start: item.start,
          end: item.start + item.duration,
          text: item.text,
          metadata: metadata ? {
            source: metadata.isAutoGenerated ? 'youtube-generated' as const : 'youtube' as const,
            quality: metadata.isAutoGenerated ? 'low' as const : 'high' as const,
    ...
    return {
      ...
      videoId: id,
      title: resolvedTitle,
      subtitleMetadata: subtitles[0]?.metadata
    };
  };
