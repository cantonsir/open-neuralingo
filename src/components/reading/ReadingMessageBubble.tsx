...
interface ReadingMessageBubbleProps {
    role: 'user' | 'assistant';
    text?: string;
    session?: ReadingSession | null;
    isTyping?: boolean;
    // For rendering the session card content
    renderSessionCard?: (session: ReadingSession) => React.ReactNode;
}

export default function ReadingMessageBubble({ role, text, session, isTyping, renderSessionCard }: ReadingMessageBubbleProps) {
        ...
        return (
            <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'} mb-6 animate-in fade-in slide-in-from-bottom-2`}>
                <div className={`flex max-w-[80%] md:max-w-[70%] gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                    {/* Avatar */}
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isUser ? 'bg-indigo-100 text-indigo-600' : 'bg-blue-100 text-blue-600'}`}>
